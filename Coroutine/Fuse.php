...
class Fuse
{
    ...
    {
        self::$instance = $this;
        $this->THRESHOLD = getInstance()->config->get('fuse.threshold', 0.01);
        $this->CHECKTIME = getInstance()->config->get('fuse.checktime', 2000);
        $this->TRYTIME = getInstance()->config->get('fuse.trytime', 1000);
        $this->TRYMAX = getInstance()->config->get('fuse.trymax', 3);
    }
